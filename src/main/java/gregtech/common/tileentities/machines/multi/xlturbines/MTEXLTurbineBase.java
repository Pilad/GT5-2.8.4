...
public abstract class MTEXLTurbineBase extends MTEExtendedPowerMultiBlockBase<MTEXLTurbineBase>
    ...
    private static final int TURBINE_HOLDER_WINDOW_ID = 1000;

    @Override
    public void addUIWidgets(ModularWindow.Builder builder, UIBuildContext buildContext) {
        // Вызываем родительский метод, чтобы отрисовались стандартные элементы GUI
        super.addUIWidgets(builder, buildContext);

        // Регистрируем всплывающее окно в контексте
        buildContext.addSyncedWindow(TURBINE_HOLDER_WINDOW_ID, (player) -> createTurbineHolderWindow());

        // Создаем кнопку-шестеренку
        builder.widget(new ButtonWidget().setOnClick((clickData, widget) -> {
            if (!widget.isClient()) {
                // Открываем или закрываем окно при нажатии
                if (widget.getContext()
                    .isWindowOpen(TURBINE_HOLDER_WINDOW_ID)) {
                    widget.getContext()
                        .closeWindow(TURBINE_HOLDER_WINDOW_ID);
                } else {
                    widget.getContext()
                        .openSyncedWindow(TURBINE_HOLDER_WINDOW_ID);
                }
            }
        })
            ...
            .setBackground(() -> {
                ...
                ret.add(GTUITextures.BUTTON_GEAR); // Обычный фон кнопки
                // В ModularUI1 нет встроенной текстуры шестеренки как в MUI2.
                // Можно использовать любую подходящую оверлейную текстуру, например:
                // ret.add(GTUITextures.OVERLAY_BUTTON_POWER_PANEL);
                // Или оставить кнопку пустой (только фон).
                return ret.toArray(new IDrawable[0]);
            })
            .dynamicTooltip(() -> {
                List<String> tooltip = new ArrayList<>();
                tooltip.add(StatCollector.translateToLocal("GT5U.gui.text.button.turbinemenu"));
                return tooltip;
            })
            .setTooltipShowUpDelay(TOOLTIP_DELAY)
            // Позиция кнопки (подберите под свой интерфейс, например 174, 116)
            .setPos(174, 91)
            .setSize(18, 18));
    }

    private ModularWindow createTurbineHolderWindow() {
        // Размеры окна: 4 слота * 18px = 72. Плюс отступы по краям = ~100 ширину
        // 3 слота * 18px = 54. Плюс заголовок (18) и отступы = ~100 высоты
        ModularWindow.Builder windowBuilder = ModularWindow.builder(100, 100);

        windowBuilder
            // Стандартный фон (серый, как у обычных инвентарей)
            .setBackground(GTUITextures.BACKGROUND_SINGLEBLOCK_DEFAULT)
            // Размещаем окно справа от основного GUI
            .setPos(
                (screenSize, mainWindow) -> new Pos2d(
                    mainWindow.getPos().x + mainWindow.getSize().width + 5,
                    mainWindow.getPos().y))
            // Заголовок "Turbines"
            .widget(
                new TextWidget(StatCollector.translateToLocal("GT5U.gui.text.button.turbinemenu"))
                    .setDefaultColor(0xFFFFFF) // Белый
                    // цвет
                    // текста
        ...
        int startX = 14; // Отступ слева для центрирования слотов
        int startY = 24; // Отступ сверху (под заголовком)

        // Создаем 12 слотов (3 ряда по 4 штуки)
        for (int i = 0; i < TURBINE_SLOTS; i++) {
            int col = i % 4;
            int row = i / 4;
            int finalI = i;

            windowBuilder.widget(new SlotWidget(new BaseSlot(turbineHolder, finalI) {

                @Override
                public boolean isItemValid(ItemStack stack) {
                    // Фильтрация: разрешаем только валидные турбины
                    return isValidTurbine(stack);
                }
            }).setPos(startX + col * 18, startY + row * 18)
                .setSize(18, 18));
        }
